...
class ApertureSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['orientation_deg', 'semi_major_axis_arcsec',
                  'semi_minor_axis_arcsec', 'type', 'cutout']
...
def serialize_blast_science_data(transient_name) -> dict:
    ...
    """
    serializers = [TransientSerializer, HostSerializer, ApertureSerializer,
                   ApertureSerializer]
    prefixes = ['transient_', 'host_', 'local_aperture_', 'global_aperture_']
    queries = [{"name__exact": transient_name}, {"transient__name__exact": transient_name},
    ...
    for filter in models.Filter.objects.all():
        for type in ['local', 'global']:
            ...
            queries.append({"transient__name__exact": transient_name,
    ...
    for model, serializer, prefix, query in zip(blast_models, serializers, prefixes, queries):
        try:
