...
def main(
    ...
    for d in data:
        ...
        with open("slurm_template.sh", "r") as fin, open(slurmfile, "w") as fout:
            for line in fin:
                line = (
                    line.replace("\n", "")
                ...
                if "transient_redshift" in d.keys():
                    line = line.replace(
                        "<transient_redshift>", str(d["transient_redshift"])
                    )
                else:
                    line = line.replace(
                        "export BLAST_TRANSIENT_REDSHIFT=<transient_redshift>", ""
                    )
