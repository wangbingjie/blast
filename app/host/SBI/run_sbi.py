...
sbi_params = {'anpe_fname' : 'host/SBI/SBI_model.pt',    # trained sbi model
...
def main():
    ...
    anpe = Inference.SNPE(
        ...
        density_estimator=Ut.posterior_nn(
            'maf', hidden_features=sbi_params['nhidden'], 
    ...
    # toy noise model
    toy_noise_x, toy_noise_y = np.loadtxt('host/SBI/snrfiles/PanSTARRS_r_magvsnr.txt', dtype=float, unpack=True)
    ...
    for f in all_filters:
        if f.name in pobs['filternames']:
            ...
            mags_unc = np.append(mags_unc, 2.5/np.log(10)*pobs['maggies_unc'][iflt]/pobs['maggies'][iflt])
        else:
