...
def sbi_missingband(obs, run_params, sbi_params, seconditer=False):
    ...
    while cnt < run_params["nmc"]:
        ### hack!
        signal.alarm(0) #run_params["tmax_per_obj"])  # max time spent on one object in sec
        try:
            x = np.copy(observed)
            # D. Jones edit            
            ...
            for j, idx in enumerate(not_valid_idx):  ##range(len(not_valid_idx)):
                x[not_valid_idx[j]] = np.random.choice(guess_ndata.T[j]) #np.clip(kdes[j].resample(size=1),0,23) ### I think these crazy non-detections are destroying the fit?
                # hack!
            ...
            if not 'hi':
                ...
                for i in range(1):
                    print(i)
                    if i == 0:
                        best_spec, best_phot, mfrac = model_components.predict( #["model"]
	                        noiseless_theta[i,:], obs=observations, sps=sps #model_components["sps"]
                        )
                        ...
                        best_spec, best_phot_s, mfrac = model_components.predict( #["model"]
	                        noiseless_theta[i,:], obs=observations, sps=sps #model_components["sps"]
                        )
                        noiseless_theta[i,1] -= np.log10(mfrac)
                        best_spec, best_phot, mfrac = model_components.predict( #["model"]
