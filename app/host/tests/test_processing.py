...
from ..processing import TaskRunner, update_status, GhostRunner
...
class TaskRunnerTest(TestCase):
    ...
    def test_update_status_status_change(self):
        register_item = TaskRegister.objects.get(transient__name__exact='2022testtwo',
                                                 task__name__exact='Host match')
...
class GHOSTRunnerTest(TestCase):
    ...
    def test_failed_status(self):
        self.assertTrue(
            self.ghost_runner.failed_status.message == 'no GHOST match')


class InitializeTaskRegisterTest(TestCase):
    fixtures = ['setup_test_transient.yaml', 'setup_status.yaml',
                'setup_tasks.yaml']
