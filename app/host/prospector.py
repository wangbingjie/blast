...
def get_CI(chain):
    ...
    return chainsort[int(chainlen*0.16)],chainsort[int(chainlen*0.50)],chainsort[int(chainlen*0.84)]
...
def prospector_result_to_blast(
    ...
    writer.write_hdf5(
        hdf5_file, {}, model_components['model'],
        observations,
        prospector_output["sampling"][0], None,
    ...
    # logmass, age, tau
    logmass = np.log10(resultpars['chain'][...,np.where(np.array(resultpars['theta_labels']) == 'mass')[0][0]])
    logmass16,logmass50,logmass84 = get_CI(logmass)
    age = resultpars['chain'][...,np.where(np.array(resultpars['theta_labels']) == 'tage')[0][0]]
    age16,age50,age84 = get_CI(age)
    tau = resultpars['chain'][...,np.where(np.array(resultpars['theta_labels']) == 'tau')[0][0]]
    tau16,tau50,tau84 = get_CI(tau)

    # sfr, ssfr
    sfr = psi_from_sfh(
        resultpars['chain'][...,np.where(np.array(resultpars['theta_labels']) == 'mass')[0][0]],
        resultpars['chain'][...,np.where(np.array(resultpars['theta_labels']) == 'tage')[0][0]],
        resultpars['chain'][...,np.where(np.array(resultpars['theta_labels']) == 'tau')[0][0]])
    logsfr = np.log10(sfr)    
    ...
    prosp_results = {'host':transient.host,
                     'aperture':aperture,
                     'posterior':hdf5_file,
