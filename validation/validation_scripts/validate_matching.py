...
for _, match in validation_data.iterrows():
    ...
    transient_position = SkyCoord(ra=transient_ra, dec=transient_dec, unit=(u.hourangle, u.deg))
...
results = {"transient_ra_deg": transient_ra_deg, "transient_dec_deg": transient_dec_deg, "host_ra_deg": host_ra_deg,
           "host_dec_deg": host_dec_deg, "predicted_host_ra_deg": predicted_host_ra_deg,
           "predicted_host_dec_deg": predicted_host_dec_deg}

pd.DataFrame(results).to_csv("validation_results/matching_ghost_jones+2018.csv", index=False)
